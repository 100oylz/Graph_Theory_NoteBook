...
- [[Euler图]] — 经过每条边恰好一次的图，与正则图、完全图密切相关
- [[Hamilton图]] — 经过每个顶点恰好一次的图，与完全图、平面图密切相关
- **与完全图的关系**：$K_n$（$n$ 奇）是 Euler 图；$K_n$（$n\geq 3$）是 Hamilton 图
- **与正则图的关系**：偶数度正则连通图是 Euler 图

> 更多特殊图汇总见：[[MOC-特殊图]]

## 本章所属概念主题

- [[MOC-遍历]] — Euler遍历、Hamilton遍历、环游问题

## 跨章节关联

| 本章概念 | 关联章节 | 关联概念 |
|---------|---------|---------|
| Euler图 | 第1章 | 度序列（全偶度判定）、握手定理 |
| Euler图 | 第3章 | 连通度（Euler图必连通） |
| Hamilton图 | 第1章 | 完全图（$K_n$ 都是H图）、度序列 |
| Hamilton图 | 第3章 | 连通度（$\kappa\geq 2$ 是必要条件） |
| Hamilton图 | 第6章 | 平面性判定（利用Hamilton圈简化） |
| 旅行售货员问题 | 第1章 | 完全图（$K_n$ 赋权图） |
| 旅行售货员问题 | 第2章 | 最小生成树（近似算法） |
| 中国邮递员问题 | 第1章 | 最短路 |
